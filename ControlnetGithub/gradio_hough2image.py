...
from ControlnetGithub.annotator.util import resize_image, HWC3
from ControlnetGithub.annotator.mlsd import MLSDdetector
from ControlnetGithub.cldm.model import create_model, load_state_dict
from ControlnetGithub.cldm.ddim_hacked import DDIMSampler

apply_mlsd = MLSDdetector()

model = create_model('models/cldm_v15.yaml').cpu()
model.load_state_dict(load_state_dict('ControlnetGithub/models/control_sd15_mlsd.pth', location='cuda'))
model = model.cuda()
ddim_sampler = DDIMSampler(model)


def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps,
            guess_mode, strength, scale, seed, eta, value_threshold, distance_threshold):
    with torch.no_grad():
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                value_threshold = gr.Slider(label="Hough value threshold (MLSD)", minimum=0.01, maximum=2.0, value=0.1,
                                            step=0.01)
                distance_threshold = gr.Slider(label="Hough distance threshold (MLSD)", minimum=0.01, maximum=20.0,
                                               value=0.1, step=0.01)
