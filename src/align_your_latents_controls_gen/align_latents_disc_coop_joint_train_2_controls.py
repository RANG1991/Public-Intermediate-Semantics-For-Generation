...
from src.align_your_latents_controls_gen.align_latents_disc_coop_joint_pipeline import \
    StableDiffusionPipelineTwoControlsDisc
import re
...
from src.utils.dcdiscriminator import DiscriminatorVanilla
...
def run_validation_pipeline(disc,
                            vaes_list,
                            unets_list,
                            args,
                            accelerator,
    ...
    pipeline_validation_two_controls = StableDiffusionPipelineTwoControlsDisc.from_pretrained(
    ...
    for j in range(math.ceil(len(list_prompts_1) / num_image_per_run)):
        ...
        prompts_input = [list_prompts_1_curr, list_prompts_2_curr]
...
def main(args, trial=None):
    ...
    noise_scheduler = DDIMScheduler.from_pretrained(args.pretrained_model_name_or_path,
                                                    subfolder="scheduler")
    noise_scheduler.config.num_train_timesteps = 50
    ...
    disc = DiscriminatorVanilla(img_shape=(8, 64, 64), dtype=weight_dtype)
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                while len(weights) > 0:
                    weights.pop()
                    model = models[i]
                    if isinstance(model, DiscriminatorVanilla):
                        torch.save(model.state_dict(),
                                   os.path.join(output_dir, f"disc.pt"))
                    i -= 1
            ...
            while len(models) > 0:
                ...
                if isinstance(model, DiscriminatorVanilla):
                    model.load_state_dict(
                        torch.load(os.path.join(input_dir, f"disc.pt")))
    ...
    unet_control_1.requires_grad_(False)
    unet_control_2.requires_grad_(False)
    ...
    # if accelerator.unwrap_model(disc).dtype != torch.float32:
    #     raise ValueError(
    #         f"Controlnet loaded as datatype {accelerator.unwrap_model(disc).dtype}. {low_precision_error_string}"
    #     )
    ...
    if args.overfit_on_single_image:
        ...
        train_dataset = InterSemDataset(split="train", dataset_to_use="COCO", dataset_size=args.dataset_size,
                                        sampling_mode=True)
    val_dataset = InterSemDataset(split="validation", dataset_to_use="COCO", dataset_size=20, sampling_mode=True)
    ...
    # Optimizer creation
    params_to_optimize = disc.parameters()
    ...
    disc, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(disc, optimizer, train_dataloader,
                                                                          lr_scheduler)
    ...
    unet_control_1.to(accelerator.device, dtype=weight_dtype)
    unet_control_2.to(accelerator.device, dtype=weight_dtype)
    ...
    pipeline_validation_control_1 = StableDiffusionPipeline.from_pretrained(
        args.pretrained_model_name_or_path,
        safety_checker=None,
        unet=unet_control_1,
        vae=vae_control_1,
    ...
    pipeline_validation_control_1.scheduler = DDIMScheduler.from_config(pipeline_validation_control_1.scheduler.config)
    pipeline_validation_control_1 = pipeline_validation_control_1.to(accelerator.device)
    pipeline_validation_control_1.set_progress_bar_config(leave=True)
    pipeline_validation_control_1.set_progress_bar_config(position=0)

    pipeline_validation_control_2 = StableDiffusionPipeline.from_pretrained(
        args.pretrained_model_name_or_path,
        safety_checker=None,
        unet=unet_control_2,
        vae=vae_control_2,
    ...
    pipeline_validation_control_2.scheduler = DDIMScheduler.from_config(pipeline_validation_control_2.scheduler.config)
    pipeline_validation_control_2 = pipeline_validation_control_2.to(accelerator.device)
    pipeline_validation_control_2.set_progress_bar_config(leave=True)
    pipeline_validation_control_2.set_progress_bar_config(position=0)

    for epoch in range(first_epoch, args.num_train_epochs):
        logger.info(f"epoch num: {epoch}")
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(disc):
                control_1_images_transformed = torch.stack([image_transforms_original_controlnet(
                    cond_image.convert("RGB"), resolution=args.resolution).to(accelerator.device).to(weight_dtype)
                                                            for cond_image in batch[f"control_1"]])
                control_2_images_transformed = torch.stack([image_transforms_original_controlnet(
                    cond_image.convert("RGB"), resolution=args.resolution).to(accelerator.device).to(weight_dtype)
                                                            for cond_image in batch[f"control_2"]])
                latents_control_1 = vae_control_1.encode(control_1_images_transformed).latent_dist.sample()
                latents_control_2 = vae_control_2.encode(control_2_images_transformed).latent_dist.sample()
                # Sample a random timestep for each image
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (args.train_batch_size,),
                                          device=accelerator.device)
                timesteps = timesteps.long()
                # Sample noise that we'll add to the latents
                noise_control_1 = torch.randn_like(latents_control_1)
                noise_control_2 = torch.randn_like(latents_control_2)
                # Add noise to the latents according to the noise magnitude at each timestep
                # (this is the forward diffusion process)
                noisy_latents_control_1 = noise_scheduler.add_noise(latents_control_1, noise_control_1, timesteps)
                noisy_latents_control_2 = noise_scheduler.add_noise(latents_control_2, noise_control_2, timesteps)
                model_pred_control_1 = unet_control_1(
                    sample=noisy_latents_control_1,
                    timestep=timesteps,
                    encoder_hidden_states=batch[f"txt_embeddings_unet_control_1"].to(dtype=weight_dtype),
                ).sample

                model_pred_control_2 = unet_control_2(
                    sample=noisy_latents_control_2,
                    timestep=timesteps,
                    encoder_hidden_states=batch[f"txt_embeddings_unet_control_2"].to(dtype=weight_dtype),
                ).sample

                fake_examples = torch.cat([noisy_latents_control_1, noisy_latents_control_2], dim=1)
                real_examples = torch.cat([noisy_latents_control_1, noisy_latents_control_2], dim=1)
                real_examples.requires_grad_(True)

                disc_res_real_examples = disc(real_examples, timesteps)
                disc_res_fake_examples = disc(fake_examples, timesteps)
                loss_disc = (torch.log(disc_res_fake_examples) + torch.log(1 - disc_res_real_examples))

                sqrt_one_minus_alpha_prod = (1 - noise_scheduler.alphas_cumprod[timesteps]) ** 0.5
                sqrt_one_minus_alpha_prod = sqrt_one_minus_alpha_prod.flatten()
                torch.log(disc_res_real_examples / (1 - disc_res_real_examples)).backward(retain_graph=True)
                grad_disc_real_examples = real_examples.grad
                loss_denoise_1 = F.mse_loss(noise_control_1, model_pred_control_1 + sqrt_one_minus_alpha_prod * (
                    grad_disc_real_examples.chunk(2, dim=1)[0]))
                loss_denoise_2 = F.mse_loss(noise_control_2, model_pred_control_2 + sqrt_one_minus_alpha_prod * (
                    grad_disc_real_examples.chunk(2, dim=1)[1]))

                loss = loss_disc + 1 * (loss_denoise_1 + loss_denoise_2)
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        with torch.no_grad():
                            curr_loss = run_validation_pipeline(
                                disc=disc,
                                vaes_list=torch.nn.ModuleList(vaes_list),
                                unets_list=torch.nn.ModuleList([unet_control_1, unet_control_2]),
                                args=args,
                                accelerator=accelerator,
