...
from src.utils.project_utils import conditioning_image_transforms_original_controlnet, \
    image_transforms_original_controlnet, get_last_checkpoint_dir, remove_checkpoints_and_save_new_checkpoint, \
    compute_FID_metrics
from src.align_your_latents_controls_gen.align_latents_sample_2_controls import load_unet_unified
...
from src.align_your_latents_controls_gen.align_latents_train_2_controls import CONTROLNET_PATH
...
def run_validation_pipeline(our_pipeline_controlnet,
                            controlnet,
                            args,
                            accelerator,
                            weight_dtype,
    ...
    for j in range(math.ceil(len(list_prompts_1) / num_image_per_run)):
        ...
        with torch.autocast("cuda"):
            ...
            pipeline_output = our_pipeline_controlnet(
...
def create_pipelines_for_controls_generation(args, dtype, vae_control_1, vae_control_2, unet_unified):
...
def main(args, trial=None):
    ...
    unet_model_unified = load_unet_unified(control_type_1=args.control_type_1,
                                           control_type_2=args.control_type_2,
                                           unet_control_1=unet_control_1,
                                           unet_control_2=unet_control_2)

    our_pipeline_controlnet = create_pipelines_for_controls_generation(args=args,
                                                                       vae_control_1=vae_control_1,
                                                                       vae_control_2=vae_control_2,
                                                                       unet_unified=unet_model_unified,
                                                                       dtype=weight_dtype)
    our_pipeline_controlnet.to(accelerator.device)
    our_pipeline_controlnet.set_progress_bar_config(disable=True)
                ...
                while len(weights) > 0:
                    ...
                    if isinstance(model, MultiControlNetModel):
                        torch.save(model.state_dict(),
                                   os.path.join(output_dir, f"controlnet.pt"))
                    i -= 1
            ...
            while len(models) > 0:
                ...
                if isinstance(model, MultiControlNetModel):
                    model.load_state_dict(
                        torch.load(os.path.join(input_dir, f"controlnet.pt")))
    ...
    unet_model_unified.requires_grad_(False)
    ...
    val_dataset = torch.utils.data.Subset(val_dataset, list(range(0, 300, 15)))
    ...
    controlnet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(controlnet,
                                                                                optimizer,
                                                                                train_dataloader,
                                                                                lr_scheduler)
            ...
            with accelerator.accumulate(controlnet):
                with torch.no_grad(), torch.autocast("cuda"):
                    ...
                    for prompt in batch["text"]:
                        generator = torch.Generator(device=accelerator.device).manual_seed(1)
                        (control_1_pred,), (control_2_pred,) = our_pipeline_controlnet(
                            prompts=[dict_control_type_to_pref[args.control_type_1] + prompt.lower(),
                                     dict_control_type_to_pref[args.control_type_2] + prompt.lower(),
                                     prompt.lower()],
                        ...
                        control_1_pred_list.append(
                            conditioning_image_transforms_original_controlnet(control_1_pred).to(dtype=weight_dtype).to(
                                accelerator.device))
                        control_2_pred_list.append(
                            conditioning_image_transforms_original_controlnet(control_2_pred).to(dtype=weight_dtype).to(
                                accelerator.device))
                    control_1_pred_tensor = torch.stack(control_1_pred_list)
                    control_2_pred_tensor = torch.stack(control_2_pred_list)

                images_transformed = torch.stack(
                    [image_transforms_original_controlnet(orig_image).to(dtype=weight_dtype).to(accelerator.device)
                     for orig_image in batch["pixel_values"]])

                # Convert images to latent space
                latents = vae.encode(images_transformed).latent_dist.sample()
                ...
                down_block_res_samples, mid_block_res_sample = controlnet(
                    ...
                    encoder_hidden_states=batch["txt_embeddings_unet_unified"].to(dtype=weight_dtype).to(
                        accelerator.device),
                    controlnet_cond=[control_1_pred_tensor, control_2_pred_tensor],
                    return_dict=False,
                )
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        with torch.no_grad():
                            curr_loss = run_validation_pipeline(
                                ...
                                our_pipeline_controlnet=our_pipeline_controlnet,
                                args=args,
                                accelerator=accelerator,
...
if __name__ == "__main__":
    ...
    args.checkpoint_prefix = f"multi-controlnet-generation-checkpoint-{args.control_type_1}-{args.control_type_2}"
    if args.run_optuna:
