...
from src.utils.project_utils import conditioning_image_transforms_original_controlnet, \
    ...
    remove_checkpoints_and_save_new_checkpoint, plot_images_and_prompt, split_prompt
...
negative_prompt = "lowres, bad anatomy, worst quality, low quality, artifacts distorted"

dict_control_type_to_pref = {"seg": "A segmentation map of: ",
                             "depth": "A depth map of: ",
                             "canny": "A canny edges image of: ",
                             "hed": "A Hough edge detection image of: "}
...
def run_validation_pipeline(vae,
    ...
    pipeline_validation = StableDiffusionControlGenPipeline.from_pretrained(
    ...
    pipeline_validation.scheduler = DDIMScheduler.from_config(pipeline_validation.scheduler.config)
    pipeline_validation.set_progress_bar_config(leave=True)
    pipeline_validation.set_progress_bar_config(position=0)
    ...
    list_ground_truth_controls = []
    list_ground_truth_images = []
    list_prompts = []
    for idx, batch_val in enumerate(val_dataloader):
        list_prompts.extend(batch_val["text"])
        list_ground_truth_controls.extend(batch_val["control"])
        list_ground_truth_images.extend(batch_val["pixel_values"])
    list_prompts_control = [dict_control_type_to_pref[args.control_type] + validation_prompt.lower() for
                            validation_prompt in list_prompts]
    list_neg_prompts = [negative_prompt] * len(list_prompts)
    list_images = []
    image_logs = []
    ...
    curr_loss_val = 0.0
    for j in range(math.ceil(len(list_prompts) / num_image_per_run)):
        list_prompts_curr = list_prompts[j * num_image_per_run: (j + 1) * num_image_per_run]
        list_prompts_control_curr = list_prompts_control[j * num_image_per_run: (j + 1) * num_image_per_run]
        list_ground_truth_controls_curr = list_ground_truth_controls[j * num_image_per_run: (j + 1) * num_image_per_run]
        ...
        with torch.autocast("cuda"):
            ...
            pipeline_output = pipeline_validation(
                prompt=list_prompts_control_curr,
                negative_prompt=list_neg_prompts[j * num_image_per_run: (j + 1) * num_image_per_run],
            ...
            list_generated_controls_curr = [generated_control for generated_control in pipeline_output.control_1]

            generated_images, _ = original_controlnet_pipeline(
                image=list_generated_controls_curr,
                prompt=list_prompts_curr,
                negative_prompt=list_neg_prompts[j * num_image_per_run: (j + 1) * num_image_per_run],
                num_inference_steps=70,
                # guidance_scale=14,
                generator=generator,
            ...
            list_generated_images_curr = [generated_image for generated_image in generated_images]
            list_generated_images.extend(list_generated_images_curr)

        image_logs.append({"validation_image": list_ground_truth_controls_curr,
                           "images": list_generated_controls_curr,
                           "validation_prompt": list_prompts_curr})
        for i, curr_prompt in enumerate(list_prompts_curr):
            text_for_plot = split_prompt(curr_prompt)
            list_images.extend([list_generated_controls_curr[i],
                                list_ground_truth_controls_curr[i],
                                list_generated_images_curr[i],
                                list_ground_truth_images_curr[i]])
            list_images_titles_validation.extend([f"generated control - {text_for_plot}",
                                                  f"ground truth control - {text_for_plot}",
                                                  f"generated image - {text_for_plot}",
                                                  f"ground truth image - {text_for_plot}"])

            # curr_loss_val += ((np.asarray(dataset.get_hed(list_generated_images_curr[i])) -
            #                    np.asarray(list_generated_controls_curr[i])) ** 2).mean()
...
def collate_fn(examples, tokenizer, text_encoder, device, zero_out_prob, tokenizer_max_length, control_type):
    txt_embeddings_unet = []
    ...
    for example in examples:
        with torch.no_grad():
            ...
            tokenized_example = tokenizer(dict_control_type_to_pref[control_type] + example["text"].lower(),
                                          max_length=tokenizer_max_length,
                                          padding='max_length',
                                          truncation=True,
                                          return_tensors='pt').to(device)
            attention_masks.append(tokenized_example["attention_mask"])
            inputs_ids.append(tokenized_example["input_ids"])
            # prompt encoding
            embeddings_text_encoder = text_encoder(input_ids=tokenized_example["input_ids"],
            ...
            if zero_out_unet_text_embeddings:
                txt_embeddings_unet.append(embeddings_text_encoder_empty_string.squeeze(0))
            else:
                txt_embeddings_unet.append(embeddings_text_encoder.squeeze(0))
            # negative prompt
    ...
    return {
        ...
        "txt_embeddings_unet": torch.stack(txt_embeddings_unet),
        "control": [example[f"{control_type}"] for example in examples],
...
def main(args, trial=None):
    ...
    vae = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path,
                                        ...
                                        revision=args.revision,
                                        variant=args.variant)
    checkpoint = get_last_checkpoint_dir(output_dir=args.output_dir, prefix=f"vae-{args.control_type}-checkpoint",
                                         accelerator=accelerator)
    vae.load_state_dict(torch.load(f"{checkpoint}/vae.pt"), strict=True)
    vae.name = "vae"
    ...
    # val_dataset = torch.utils.data.Subset(val_dataset, list(range(0, 20)))
    ...
    # if accelerator.is_main_process:
    #     with torch.no_grad():
    #         _ = run_validation_pipeline(
    #             vae=vae,
    #             vae_control_1=vae_control_1,
    #             unet=unet,
    #             unet_control_1=unet_control_1,
    #             noise_scheduler_control_1=noise_scheduler_control_1,
    #             text_encoder=text_encoder,
    #             tokenizer=tokenizer,
            ...
            with accelerator.accumulate(unet):
                ...
                control_1_images_transformed = torch.stack([image_transforms_original_controlnet(
                    cond_image.convert("RGB"), resolution=args.resolution).to(accelerator.device).to(weight_dtype)
                                                            for cond_image in batch["control"]])

                latents = vae.encode(control_1_images_transformed).latent_dist.sample()
                ...
                model_pred = unet(
                    noisy_latents,
                    timesteps,
                    encoder_hidden_states=batch["txt_embeddings_unet"].to(dtype=weight_dtype),
                ).sample

                # image_processor_control_1 = VaeImageProcessor(vae_scale_factor=vae.config.scaling_factor)
                # decoded_image = vae.decode(latents / vae.config.scaling_factor).sample

                # control_1_image = decoded_image[0].unsqueeze(0).detach()
                # control_1_image = image_processor_control_1.postprocess(control_1_image, output_type="pil",
                #                                                         do_denormalize=[True])
                # control_1_image[0].save(f"{top_dir}/InterSem/test_imgs/check_control_1.png")
...
def parse_args():
    ...
    parser.add_argument('--control_type',
                        choices=["seg", "hed", "depth"],
                        help='the control type (i.e. seg / hed / depth)',
                        type=str.lower,
                        required=True)
    ...
    if args.resolution % 8 != 0:
        raise ValueError(
            ...
            "controlnet encoder.")
    args.control_type = command_args.control_type
    return args
...
if __name__ == "__main__":
    ...
    args.checkpoint_prefix = f"control-generation-checkpoint-{args.control_type}"
    args.controlnet_path = (
        f"thibaud/controlnet-sd21-{args.control_type}-diffusers" if args.control_type in ["hed", "depth"]
        else f"thibaud/controlnet-sd21-ade20k-diffusers" if args.control_type == "seg" else None)
    if args.run_optuna:
