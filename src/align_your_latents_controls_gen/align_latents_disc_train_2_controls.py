...
from src.utils.project_utils import conditioning_image_transforms_original_controlnet, \
    ...
    remove_checkpoints_and_save_new_checkpoint
...
from src.utils.dcdiscriminator import DiscriminatorVanilla, DiscriminatorCycleGAN, DCDiscriminator
...
CONTROLNET_PATH = ["thibaud/controlnet-sd21-depth-diffusers", "thibaud/controlnet-sd21-hed-diffusers"]
...
def run_validation_pipeline(vaes_list,
    ...
    controlnet_checkpoint_paths = CONTROLNET_PATH
    controlnet_list = []
    ...
    for j in range(math.ceil(len(list_prompts_1) / num_image_per_run)):
        ...
        prompts_input = [list_prompts_1_curr, list_prompts_2_curr, list_prompts_unified_curr]
        with torch.autocast("cuda"):
            ...
            pipeline_output = pipeline_validation_two_controls(
                ...
                # negative_prompt=list_neg_prompts,
    ...
    curr_loss_val = compute_FID_metrics(fake_images=list_generated_images,
                                        real_images=list_ground_truth_images,
...
def main(args, trial=None):
    ...
    accelerator = Accelerator(
        ...
        kwargs_handlers=[DistributedDataParallelKwargs(gradient_as_bucket_view=True, broadcast_buffers=False)]
    )
    ...
    vae_control_1 = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path,
                                                  ...
                                                  revision=args.revision,
                                                  variant=args.variant)
    checkpoint = get_last_checkpoint_dir(output_dir=args.output_dir, prefix=f"vae-{args.control_type_1}-checkpoint",
                                         accelerator=accelerator)
    ...
    vae_control_2 = AutoencoderKL.from_pretrained(args.pretrained_model_name_or_path,
                                                  ...
                                                  revision=args.revision,
                                                  variant=args.variant)
    checkpoint = get_last_checkpoint_dir(output_dir=args.output_dir, prefix=f"vae-{args.control_type_2}-checkpoint",
                                         accelerator=accelerator)
    ...
    unet.load_state_dict(torch.load(f"{Path(__file__).parent}/unet-to-load-disc/unet.pt"), strict=True)

    disc = DCDiscriminator(img_shape=(4, 64, 64), dtype=weight_dtype)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
            ...
            if accelerator.is_main_process:
                ...
                while len(weights) > 0:
                    ...
                    if isinstance(model, UnetConditionModelUnified):
                        torch.save(model.state_dict(), os.path.join(output_dir, f"unet.pt"))
                    elif isinstance(model, DCDiscriminator):
                        torch.save(model.state_dict(), os.path.join(output_dir, f"disc.pt"))
                    i -= 1
        ...
        def load_model_hook(models, input_dir):
            while len(models) > 0:
                model = models.pop()
                if isinstance(model, UnetConditionModelUnified):
                    model.load_state_dict(torch.load(os.path.join(input_dir, f"unet.pt")))
                elif isinstance(model, DCDiscriminator):
                    model.load_state_dict(torch.load(os.path.join(input_dir, f"disc.pt")))
    ...
    if args.overfit_on_single_image:
        ...
        train_dataset = InterSemDataset(split="train", dataset_to_use="COCO", dataset_size=args.dataset_size,
                                        sampling_mode=True)
    val_dataset = InterSemDataset(split="validation", dataset_to_use="COCO", dataset_size=300, sampling_mode=True)
    val_dataset = torch.utils.data.Subset(val_dataset, list(range(0, 300, 30)))
    ...
    # Optimizer creation
    params_to_optimize_unet = unet.parameters()
    ...
    params_to_optimize_disc = disc.parameters()
    ...
    unet, optimizer_unet, train_dataloader, lr_scheduler, disc, optimizer_disc = accelerator.prepare(unet,
                                                                                                     optimizer_unet,
                                                                                                     train_dataloader,
                                                                                                     lr_scheduler,
                                                                                                     disc,
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            if Path(f"{load_loss_path}/loss.json").exists():
                ...
                with open(f"{load_loss_path}/loss.pt") as f:
                    best_loss = json.load(f)["best_loss"]
    ...
    losses_dict_training_all_steps = {f"gen_loss": [], "rec_loss": [], "disc_loss_fake": [], "disc_loss_real": []}
    losses_dict_validation_all_steps = {f"rec_loss": []}
    losses_dict_single_step = {f"gen_loss": [], "rec_loss": [], "disc_loss_fake": [], "disc_loss_real": []}
    ...
    vae_scale_factor = 2 ** (len(vae_control_1.config.block_out_channels) - 1)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(disc):
                disc_loss_real = 0.0
                for i in range(2):
                    disc_output_original_control = disc(latents_list[i])
                    one_hot_real = torch.zeros((args.train_batch_size, 3), device=accelerator.device)
                    one_hot_real[:, i] = 1.0
                    disc_loss_real += F.cross_entropy(disc_output_original_control, one_hot_real)
                accelerator.backward(disc_loss_real)

            # discriminator loss on fake images
            with accelerator.accumulate(disc):
                disc_loss_fake = 0.0
                pred_original_image_list = []
                for i in range(2):
                    pred_original_image = noise_scheduler.step(model_pred[i], timesteps_list[i],
                                                               samples_list[i]).pred_original_sample
                    pred_original_image_list.append(pred_original_image)

                    # control_to_plot = vaes_list[i].decode(
                    #     pred_original_image.to(weight_dtype).detach() / vaes_list[i].config.scaling_factor).sample
                    # control_to_plot = image_processor.postprocess(control_to_plot, output_type="pil",
                    #                                               do_denormalize=[True])
                    # control_to_plot[0].save("check.png")

                    disc_output_pred_image = disc(pred_original_image.detach())
                    one_hot_fake = torch.zeros((args.train_batch_size, 3), device=accelerator.device)
                    one_hot_fake[:, 2] = 1.0
                    disc_loss_fake += F.cross_entropy(disc_output_pred_image, one_hot_fake)
                accelerator.backward(disc_loss_fake)
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(params_to_optimize_disc, args.max_grad_norm)
                optimizer_disc.step()

            # generator (unet) loss on fake images
            with accelerator.accumulate(unet):
                unet_loss_disc = 0.0
                for i in range(2):
                    disc_output_pred_image = disc(pred_original_image_list[i])
                    one_hot_real = torch.zeros((args.train_batch_size, 3), device=accelerator.device)
                    one_hot_real[:, i] = 1.0
                    unet_loss_disc += F.cross_entropy(disc_output_pred_image, one_hot_real)
                unet_loss = unet_loss_disc + reconstruction_loss_unet_all
                accelerator.backward(unet_loss)
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(params_to_optimize_unet, args.max_grad_norm)
                optimizer_unet.step()
                lr_scheduler.step()

            optimizer_disc.zero_grad(set_to_none=args.set_grads_to_none)
            optimizer_unet.zero_grad(set_to_none=args.set_grads_to_none)

            losses_dict_single_step[f"gen_loss"].append(unet_loss_disc.detach().item())
            losses_dict_single_step[f"rec_loss"].append(reconstruction_loss_unet_all.detach().item())
            losses_dict_single_step[f"disc_loss_fake"].append(disc_loss_fake.detach().item())
            losses_dict_single_step[f"disc_loss_real"].append(disc_loss_real.detach().item())

            if trial is not None:
                trial.report((unet_loss + disc_loss_fake + disc_loss_real).detach().item(), global_step)
            if trial is not None:
                ...
                if accelerator.is_main_process:
                    if global_step % args.validation_steps == 0:
                        ...
                        with torch.no_grad():
                            ...
                            losses_dict_training_all_steps[f"gen_loss"].append(
                                np.array(losses_dict_single_step[f"gen_loss"]).mean().item())
                            losses_dict_training_all_steps[f"rec_loss"].append(
                                np.array(losses_dict_single_step[f"rec_loss"]).mean().item())
                            losses_dict_training_all_steps[f"disc_loss_fake"].append(
                                np.array(losses_dict_single_step[f"disc_loss_fake"]).mean().item())
                            losses_dict_training_all_steps[f"disc_loss_real"].append(
                                np.array(losses_dict_single_step[f"disc_loss_real"]).mean().item())
                        ...
                        ax1.plot(losses_dict_training_all_steps[f"gen_loss"], label=f"training - generator loss")
                        ax1.plot(losses_dict_training_all_steps[f"disc_loss_fake"],
                                 label=f"training - discriminator loss fake")
                        ax1.plot(losses_dict_training_all_steps[f"disc_loss_real"],
                                 label=f"training - discriminator loss real")
                        ax1.legend(loc="upper left")
                        ax1.set_title('training - GAN losses')

                        ax2.plot(losses_dict_training_all_steps[f"rec_loss"], label=f"training - reconstruction loss")
                        ax2.legend(loc="upper left")
                        ax2.set_title('training - reconstruction loss')

                        ax3.plot(losses_dict_validation_all_steps[f"rec_loss"],
                                 label=f"validation - FID loss")
                        ax3.legend(loc="upper left")
                        ax3.set_title('validation - FID loss')
                        plt.tight_layout()
            ...
            logs = {"gen_loss": unet_loss_disc.detach().item(),
                    "rec_loss": reconstruction_loss_unet_all.detach().item(),
                    "disc_loss_fake": disc_loss_fake.detach().item(),
                    "disc_loss_real": disc_loss_real.detach().item(),
...
if __name__ == "__main__":
    ...
    args.checkpoint_prefix = f"control-generation-checkpoint-disc-{args.control_type_1}-{args.control_type_2}"
    if args.run_optuna:
